    ...
    try {
      ...
      final thumbnails =
          json['thumbnail']?['musicThumbnailRenderer']?['thumbnail']?['thumbnails']
              as List?;
      // **FIX:** Modify the URL to request a more appropriate resolution image for thumbnails.
      String imageUrl = '';
      if (thumbnails != null && thumbnails.isNotEmpty) {
        ...
        if (baseImageUrl.contains('googleusercontent.com')) {
          // We replace the size parameter (e.g., "=w120-h120") with a more suitable one for grid thumbnails.
          // Request a 300x300 pixel image. Adjust this value if your grid items are significantly larger/smaller.
          final urlParts = baseImageUrl.split('=');
          if (urlParts.length > 1) {
            imageUrl = '${urlParts[0]}=w300-h300-l90-rj';
          } else {
